...
class CategoricalEnsembler(object):
    ...
    def __init__(self, trained_models, transformation_pipeline,
                 categorical_column, default_category):
...
def clean_categorical_definitions(df, categorical_column):
    ...
    if sum_of_nan_values > 0:
        print('Found ' + str(sum_of_nan_values) +
              ' nan values in the categorical_column.')
        print(
            'We will default to making these values a string "nan" instead, since that can be used as a key'
        )
        print(
            'If this is not the behavior you want, consider changing these categorical_column values yourself'
        )
