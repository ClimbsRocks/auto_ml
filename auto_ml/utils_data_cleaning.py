...
bad_vals_as_strings = set([
    str(float('nan')),
    str(float('inf')),
    str(float('-inf')), 'None', 'none', 'NaN', 'nan', 'NULL', 'null', '', 'inf',
    '-inf'
])
...
def clean_val_nan_version(key, val, replacement_val=np.nan):
    try:
        ...
        print(
            'Here is the value that causes the UnicodeEncodeError to be thrown:'
        )
        print(val)
    ...
    if str_val in bad_vals_as_strings:
        ...
        try:
            ...
            try:
                ...
                print('*************************************')
                print(
                    'We expected this value to be numeric, but were unable to convert it to a float:'
                )
                print(val)
                print('Here is the feature name:')
                print(key)
...
class BasicDataCleaning(BaseEstimator, TransformerMixin):
        ...
        for key in X_df.columns:
            ...
            if X_df[key].dtype == 'object' and self.column_descriptions.get(
                ...
                if is_categorical:
                    ...
                    print(
                        'If this is a categorical column, please mark it as `{}: "categorical"` as part of your column_descriptions'.
                        format(key))
                    print(
                    ...
                    print('\n')
                    warnings.warn(
                        'Consider marking the "{}" column as categorical'.
                        format(key))
            ...
            if key in self.text_columns:
                ...
                col_names = [
                    'nlp_' + key + '_' + str(word) for word in col_names
                ]
    ...
    def transform(self, X, y=None):
        ...
        column_descriptions = self.get('transformed_column_descriptions',
        ...
        if isinstance(X, dict):
            ...
            for key, val in X.items():
                ...
                if col_desc is None:
                    continue
                elif col_desc in (None, 'continuous', 'numerical', 'float',
                                  'int'):
                    dict_copy[key] = clean_val_nan_version(
                        key, val, replacement_val=0)
                elif col_desc == 'date':
                    ...
                    relevant_nlp_cols = {
                        k: v
                        for k, v in zip(relevant_col_names, nlp_matrix.data)
                    }
            ...
            if len(cols_to_clean) > 0:
                ...
                if df_to_clean.shape[0] > 100000 or os.environ.get(
                        'is_test_suite', 0) == 'True':
    ...
    def process_one_column(self, col_vals, col_name):
        ...
        if col_desc is None:
            ...
            try:
                ...
                print(
                    'We have found a column that is not marked as a categorical column that has unicode values in it'
                )
                print('Here is the column name:')
                print(col_name)
                print(
                    'The actual value that caused the issue is logged right above the exclamation points'
                )
                print(
                    'Please either mark this column as categorical, or clean up the values in this column'
                )
                print(
                    '!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!'
                )
            ...
            print(
                'When transforming the data, we have encountered a value in column_descriptions that is not currently supported. The column has been dropped to allow the rest of the pipeline to run. Here\'s the name of the column:'
            )
            print(col_name)
            print(
                'And here is the value for this column passed into column_descriptions:'
            )
            print(col_desc)
            warnings.warn(
                'UnknownValueInColumnDescriptions: Please make sure all the values you pass into column_descriptions are valid.'
            )
            result = {}
...
def add_date_features_df(col_data, date_col):
    ...
    if pandas_version < '0.20.0':
        result[date_col +
               '_day_of_week'] = col_data.apply(lambda x: x.weekday()).astype(
    ...
    try:
        if pandas_version < '0.20.0':
            result[date_col +
                   '_hour'] = col_data.apply(lambda x: x.hour).astype(
                       int, raise_on_error=False)
        else:
            result[date_col +
                   '_hour'] = col_data.apply(lambda x: x.hour).astype(
        ...
        result[date_col + '_minutes_into_day'] = col_data.apply(
            lambda x: x.hour * 60 + x.minute)

        result[date_col + '_hour'] = result[date_col + '_hour'].fillna(0)
        result[date_col + '_minutes_into_day'] = result[
            date_col + '_minutes_into_day'].fillna(0)
    except AttributeError:
        pass

    result[date_col +
           '_is_weekend'] = col_data.apply(lambda x: x.weekday() in (5, 6))
    result[date_col + '_day_part'] = result[
        date_col + '_minutes_into_day'].apply(minutes_into_day_parts)

    result[date_col + '_day_of_week'] = result[date_col +
                                               '_day_of_week'].fillna(0)
    result[date_col + '_is_weekend'] = result[date_col +
                                              '_is_weekend'].fillna(0)
...
def add_date_features_dict(row, date_col):
    ...
    try:
        ...
        date_feature_dict[
            date_col +
            '_minutes_into_day'] = date_val.hour * 60 + date_val.minute
    except AttributeError:
        pass
