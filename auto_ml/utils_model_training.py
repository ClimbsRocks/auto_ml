...
class FinalModelATC(BaseEstimator, TransformerMixin):
            ...
            if os.environ.get('is_test_suite', False) == 'True':
                ...
                num_iters = list(range(1, 50, 1)) + list(range(
                    50, 100, 2)) + list(range(100, 250, 3)) + list(
                        range(250, 500, 5)) + list(range(500, 1000, 10)) + list(
            ...
            try:
                for num_iter in num_iters:
                    ...
                    self.model.set_params(
                        n_estimators=num_iter, warm_start=warm_start)
    ...
    def predict_proba(self, X, verbose=False):
        ...
        if (self.model_name[:16] == 'GradientBoosting' or
                self.model_name[:12] == 'DeepLearning' or self.model_name in [
                    'BayesianRidge', 'LassoLars', 'OrthogonalMatchingPursuit',
                    'ARDRegression'
                ]):
            if scipy.sparse.issparse(X):
                ...
                X = X.values
        elif (self.model_name[:8] == 'CatBoost' or
              self.model_name[:4] == 'LGBM'):
            if scipy.sparse.issparse(X):
                X = X.toarray()
            elif isinstance(X, pd.DataFrame):
                X = X.values
        ...
        if not (hasattr(predictions[0], '__iter__') and
                not isinstance(predictions[0], str)):
            tupled_predictions = []
    ...
    def predict_uncertainty(self, X):
        ...
        X_combined = scipy.sparse.hstack(
            [X, base_predictions_col], format='csr')
    ...
    def get_X_test(self, X_fit, y):
        ...
        if self.X_test is not None:
            ...
            X_fit, X_test, y, y_test = train_test_split(
                X_fit, y, test_size=0.15)
