...
class Predictor(object):
    ...
    def __init__(self,
        ...
        if type_of_estimator.lower() in [
                'regressor', 'regression', 'regressions', 'regressors',
                'number', 'numeric', 'continuous'
        ]:
            self.type_of_estimator = 'regressor'
        elif type_of_estimator.lower() in [
                'classifier', 'classification', 'categorizer', 'categorization',
                'categories', 'labels', 'labeled', 'label'
        ]:
    ...
    def _validate_input_col_descriptions(self):
        ...
        for key, value in self.column_descriptions.items():
            ...
            if value == 'output':
                ...
                raise ValueError(
                    'We are not sure how to process this column of data: ' +
                    str(value) +
        ...
        if found_output_column is False:
            ...
            raise ValueError(
                'In your column_descriptions, please make sure exactly one column has the value "output", which is the value we will be training models to predict.'
            )
    ...
    def _construct_pipeline(self,
        ...
        if self.user_input_func is not None:
            if trained_pipeline is not None:
                pipeline_list.append(
                    ('user_func', trained_pipeline.named_steps['user_func']))
            elif self.transformation_pipeline is None:
                print(
                    'Including the user_input_func in the pipeline! Please remember to return X, and not modify the length or order of X at all.'
                )
                print(
                    'Your function will be called as the first step of the pipeline at both training and prediction times.'
                )
                pipeline_list.append(('user_func',
                                      FunctionTransformer(
                                          func=self.user_input_func,
                                          validate=False)))
        ...
        if self.perform_feature_scaling is True:
            if trained_pipeline is not None:
                pipeline_list.append(('scaler',
                                      trained_pipeline.named_steps['scaler']))
            else:
        ...
        if trained_pipeline is not None:
            ...
            for step in trained_pipeline.steps:
                ...
                if step_name[-6:] == '_model':
                    pipeline_list.append(
                        (step_name, trained_pipeline.named_steps[step_name]))
            ...
            final_model = utils_models.get_model_from_name(
                model_name, training_params=params)
            pipeline_list.append((
                'final_model',
        ...
        constructed_pipeline = utils.ExtendedPipeline(
            pipeline_list, keep_cat_features=keep_cat_features, name=self.name)
    ...
    def _get_estimator_names(self):
        if self.type_of_estimator == 'regressor':
            ...
            raise (
                'TypeError: type_of_estimator must be either "classifier" or "regressor".'
            )
    ...
    def _consolidate_pipeline(self, transformation_pipeline, final_model=None):
        ...
        try:
            feature_selection = transformation_pipeline.named_steps[
                'feature_selection']
            feature_selection_mask = feature_selection.support_mask
            transformation_pipeline.named_steps['dv'].restrict(
    ...
    def set_params_and_defaults(self,
        ...
        if self.model_names is None or (len(self.model_names) == 1 and
                                        self.model_names[0] is None):
        ...
        if 'DeepLearningRegressor' in self.model_names or 'DeepLearningClassifier' in self.model_names or feature_learning == True:
            if perform_feature_scaling is None or perform_feature_scaling == True:
                ...
                print(
                    'Heard that we should not perform feature_scaling, but we should train a Deep Learning model. Note that feature_scaling is typically useful and frequently essential for deep learning. We STRONGLY suggest not setting perform_feature_scaling=False.'
                )
                warnings.warn(
                    'It is a best practice, and often necessary for training, to perform_feature_scaling while doing Deep Learning.'
                )
        ...
        default_analytics_config = {
            ...
            'cols_to_ignore': [],
            'file_name':
            'auto_ml_analytics_results_' + self.output_column + '.csv',
            'col_std_multiplier':
            0.5
        }
        if analytics_config is None:
            ...
            updated_analytics_config = updated_analytics_config.update(
                analytics_config)
        ...
        if self.train_uncertainty_model == True and self.type_of_estimator == 'classifier':
            ...
            print(
                'Right now uncertainty predictions are only supported for regressors. The ".predict_proba()" method of classifiers is a reasonable workaround if you are looking for uncertainty predictions for a classifier'
            )
            print(
                '!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!'
            )
            raise ValueError(
                'train_uncertainty_model is only supported for regressors')
        ...
        if uncertainty_delta is not None:
            if uncertainty_delta_units is None:
                ...
                print(
                    'We received an uncertainty_delta, but do not know the units this is measured in. Please pass in one of ["absolute", "percentage"]'
                )
                print(
                    '!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!'
                )
                raise ValueError(
                    'We received a value for uncertainty_delta, but the data passed in for uncertainty_delta_units is missing'
                )
        ...
        if self.train_uncertainty_model == True and self.uncertainty_data is None:
            ...
            print(
                'Saw that train_uncertainty_model is True, but there is no data passed in for uncertainty_data, which is needed to train the uncertainty estimator'
            )
            warnings.warn(
                'Please pass in uncertainty_data which is the dataset that will be used to train the uncertainty estimator.'
            )
            print(
                '!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!'
            )
            raise ValueError(
                'The data passed in for uncertainty_data is missing')
        ...
        if self.feature_learning == True:
            if fl_data is None:
                ...
                print(
                    'Saw that feature_learning is True, but there is no data passed in for fl_data, which is needed to train the feature_learning estimator'
                )
                warnings.warn(
                    'Please pass in fl_data which is the dataset that will be used to train the feature_learning estimator.'
                )
                print(
                    '!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!'
                )
            ...
            if self.perform_feature_scaling == False:
                ...
                print(
                    'Heard that we should not perform feature_scaling, but we should perform feature_learning. Note that feature_scaling is typically useful for deep learning, which is what we use for feature_learning. If you want a little more model accuracy from the feature_learning step, consider not passing in perform_feature_scaling=False'
                )
                warnings.warn(
                    'Consider allowing auto_ml to perform_feature_scaling in conjunction with feature_learning'
                )
            ...
            if self.perform_feature_selection == True:
                ...
                print(
                    'We are not currently supporting perform_feature_selection with this release of feature_learning. We will override perform_feature_selection to False and continue with training.'
                )
                warnings.warn(
                    'perform_feature_selection=True is not currently supported with feature_learning.'
                )
            self.perform_feature_selection = False

            if (isinstance(X_df, pd.DataFrame) and
                    X_df.equals(fl_data)) or (isinstance(X_df, list) and
                                              X_df == fl_data):
                print(
                    '!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!'
                )
                print(
                    'You must pass in different data for fl_data and your training data. This is true both philosophically (you are much more likely to overfit if fl_data == training_data), and logistically (we remove the y column from both datasets, which will throw an error)'
                )
                print(
                    'If you are looking for a quick and easy way of splitting the data, use scikit-learn\'s train_test_split: df_train, fl_data = train_test_split(df_train, test_size=0.33)  '
                )
                print(
                    'Or, if you insist on using the same dataset for both, you must at least copy it:'
                )
                print(
                    'ml_predictor.train(df_train, feature_learning=True, fl_data=df_train.copy())'
                )
                warnings.warn(
                    'Your fl_data and df_train must be different datasets. Use train_test_split, or at least copy the data for your fl_data'
                )
        ...
        if trained_transformation_pipeline is None:
            ...
            print(
                'We will be using the previously trained transformation pipeline you passed in'
            )
            print(
                'Be cautious when passing in a trained transformation pipeline- make sure that it is trained on exactly the same data.'
            )
        ...
        if transformed_X is not None and transformed_y is None:
            ...
            raise (ValueError(
                'Please pass in transformed_y if you are passing in transformed_X'
            ))
    ...
    def fit_feature_learning_and_transformation_pipeline(
            self, X_df, fl_data, y):
        fl_data_cleaned, fl_y = self._clean_data_and_prepare_for_training(
            fl_data, self.scoring)
        ...
        combined_transformed_data = self.fit_transformation_pipeline(
            combined_training_data, combined_y, fl_estimator_names)

        fl_indices = [
            i for i in range(len_X_df, combined_transformed_data.shape[0])
        ]
        ...
        # TODO:
        self.transformation_pipeline.named_steps[
            'dv'].feature_names_ += feature_learning_names
    ...
    def train(self,
        ...
        if transformed_X is None:
            X_df, y = self._clean_data_and_prepare_for_training(
                raw_training_data, scoring)
            del raw_training_data

            if self.transformation_pipeline is None:
                if self.feature_learning == True:
                    X_df = self.fit_feature_learning_and_transformation_pipeline(
                        X_df, fl_data, y)
                else:
                    # If the user passed in a valid value for model_names (not None, and not a list where the only thing is None)
                    if self.model_names is not None and not (
                            len(self.model_names) == 1 and
                            self.model_names[0] is None):
        ...
        self.trained_final_model = self.train_ml_estimator(
            self.model_names, self._scorer, X_df, y)
        ...
        if self.calculate_prediction_intervals is True:
            ...
            for percentile in self.prediction_intervals:
                interval_predictor = self.train_ml_estimator(
                    ['GradientBoostingRegressor'],
                    self._scorer,
                    X_df,
                    y,
                    prediction_interval=percentile)
                predictor_tup = ('interval_{}'.format(percentile),
                                 interval_predictor)
                interval_predictors.append(predictor_tup)

            self.trained_final_model.interval_predictors = interval_predictors

        self.trained_pipeline = self._consolidate_pipeline(
            self.transformation_pipeline, self.trained_final_model)
    ...
    def _create_uncertainty_model(self, uncertainty_data, scoring, y,
                                  uncertainty_calibration_data):
        ...
        self.transformation_pipeline.named_steps['dv'].feature_names_.append(
            'base_prediction')
        ...
        uncertainty_data_transformed = self.transformation_pipeline.transform(
            uncertainty_data)

        base_predictions = self.trained_final_model.predict(
            uncertainty_data_transformed)
        ...
        is_uncertain_predictions = self.define_uncertain_predictions(
            base_predictions, y_uncertainty)
        ...
        percent_uncertain = sum(is_uncertain_predictions) * 1.0 / len(
            is_uncertain_predictions)
        print(percent_uncertain)
        if percent_uncertain == 1.0:
            print(
                'Using the current definition, all rows are classified as uncertain'
            )
            ...
            warnings.warn(
                'All predictions in ojur uncertainty training data are classified as uncertain. Please redefine uncertainty so there is a mix of certain and uncertain predictions to train an uncertainty model.'
            )
            return self
        ...
        self.trained_uncertainty_model = self.train_ml_estimator(
            uncertainty_estimator_names, self._scorer,
            uncertainty_data_transformed, is_uncertain_predictions)
        ...
        if self.calibrate_uncertainty == True:
            ...
            try:
                ...
                bucket_results, bins = pd.qcut(
                    probas, q=num_buckets, retbins=True)
            ...
            try:
                ...
                bucket_results = pd.qcut(
                    probas, q=num_buckets, labels=bucket_labels)
            ...
            for bucket in bucket_labels:
                ...
                uc_results[bucket]['max_proba'] = np.max(
                    dataset['uncertainty_prediction'])

                for perc in self.uncertainty_calibration_settings[
                        'percentiles']:
                    delta_at_percentile = np.percentile(deltas, perc)
                    uc_results[bucket]['percentile_' + str(perc) +
                                       '_delta'] = delta_at_percentile

            # make the max_proba of our last bucket_num 1
            uc_results[bucket_labels[-1]]['max_proba'] = 1
            print(
                'Here are the uncertainty_calibration results, for each bucket of predicted probabilities'
            )
    ...
    def _prepare_for_verify_features(self):
        ...
        self.trained_pipeline.set_params(
            final_model__training_features=trained_feature_names)
        # We will need to know which columns are categorical/ignored/nlp when verifying features
        self.trained_pipeline.set_params(
            final_model__column_descriptions=self.column_descriptions)

    def _calibrate_final_model(self, trained_model, X_test, y_test):

        if X_test is None or y_test is None:
            print(
                'X_test or y_test was not present while trying to calibrate the final model'
            )
            print(
                'Please pass in both X_test and y_test to calibrate the final model'
            )
            print('Skipping model calibration')
            return trained_model

        print(
            'Now calibrating the final model so the probability predictions line up with the observed probabilities in the X_test and y_test datasets you passed in.'
        )
        print(
            'Note: the validation scores printed above are truly validation scores: they were scored before the model was calibrated to this data.'
        )
        print(
            'However, now that we are calibrating on the X_test and y_test data you gave us, it is no longer accurate to call this data validation data, since the model is being calibrated to it. As such, you must now report a validation score on a different dataset, or report the validation score used above before the model was calibrated to X_test and y_test. '
        )
        ...
        try:
            calibrated_classifier = calibrated_classifier.fit(
    ...
    def fit_single_pipeline(self,
        ...
        if self.verbose:
            ...
            if prediction_interval is not False:
                print(
                    'About to fit a {} quantile regressor to predict the prediction_interval for the {}th percentile'.
                    format(model_name, int(prediction_interval * 100)))
            else:
                print('About to fit the pipeline for the model ' + model_name +
                      ' to predict ' + self.output_column)
            print('Started at:')
    ...
    def fit_transformation_pipeline(self, X_df, y, model_names):
        ...
        for model_name in model_names:
            keep_cat_features = keep_cat_features and model_name in [
                'LGBMRegressor', 'LGBMClassifier', 'CatBoostRegressor',
                'CatBoostClassifier'
            ]
    ...
    def create_feature_responses(self,
                                 model,
                                 X_transformed,
                                 y,
                                 top_features=None):
        ...
        if orig_row_count <= 10000:
            ...
            percent_row_count = int(
                self.analytics_config['percent_rows'] * orig_row_count)
        ...
        def create_one_feature_response(col_idx, col_name):
            ...
            if col_name[:
                        4] != 'nlp_' and '=' not in col_name and self.column_descriptions.get(
                ...
                col_delta = self.analytics_config[
                    'col_std_multiplier'] * col_std
    ...
    def print_results(self, model_name, model, X, y):
        ...
        try:
            if self.ml_for_analytics and model_name in ('LogisticRegression',
                                                        'RidgeClassifier',
                                                        'LinearRegression',
                                                        'Ridge'):
                df_model_results = self._print_ml_analytics_results_linear_model(
                    model)
                sorted_model_results = df_model_results.sort_values(
                    by='Coefficients', ascending=False)
                sorted_model_results = sorted_model_results.reset_index(
                    drop=True)
                # only grab the top 100 features from X
                top_features = set(
                    sorted_model_results.head(n=100)['Feature Name'])

                feature_responses = self.create_feature_responses(
                    model, X, y, top_features)
                self._join_and_print_analytics_results(
                    feature_responses,
                    sorted_model_results,
                    sort_field='Coefficients')
                ...
                try:
                    df_model_results = self._print_ml_analytics_results_random_forest(
                        model)
                    sorted_model_results = df_model_results.sort_values(
                ...
                feature_responses = feature_responses[[
                    'Feature Name', 'Delta', 'FR_Decrementing',
                    'FR_Incrementing', 'FRD_MAD', 'FRI_MAD'
                ]]
    ...
    def fit_grid_search(self,
        ...
        # However, it does, itself, parallelize pretty nicely. So let lgbm take care of the parallelization itself, which will be less memory intensive than having to duplicate the data for all the cores on the machine
                ...
                'LGBMRegressor', 'LGBMClassifier', 'DeepLearningRegressor',
        ...
        if self.verbose:
            ...
            if self.optimize_final_model == True:
                ...
                if fit_evolutionary_search == False:
                    print(
                        'About to run GridSearchCV to find the optimal hyperparameters for the model '
                        + model_name + ' to predict ' + self.output_column)
                else:
                    print(
                        'About to run EvolutionaryAlgorithmSearchCV to find the optimal hyperparameters for the model '
                        + model_name + ' to predict ' + self.output_column)
                    print('Population size each generation: ' +
                          str(population_size))
    ...
    def create_gs_params(self, model_name):
        ...
        if self.user_gs_params is not None:
            ...
            print(
                'Please note that if you want to set the grid search params for the final model specifically, they need to be prefixed with: "model__"'
            )
    ...
    def train_ml_estimator(self,
        ...
        elif (feature_learning == True and self.optimize_feature_learning !=
              True) or (len(estimator_names) == 1 and
                        self.optimize_final_model != True):
            trained_final_model = self.fit_single_pipeline(
                X_df,
                y,
        ...
        # Use Case 2: Compare a bunch of models, but don't optimize any of them
            ...
            final_model_models = map(utils_models.get_model_from_name,
                                     estimator_names)
            ...
            gscv_results = self.fit_grid_search(
                X_df, y, grid_search_params, refit=True)
        ...
        # Use Case 4: Many models, and optimize them!
        elif (feature_learning == False and self.optimize_final_model == True
             ) or (feature_learning == True and
                   self.optimize_feature_learning == True):
            ...
            for model_name in estimator_names:
                ...
                grid_search_params['model'] = [
                    utils_models.get_model_from_name(model_name)
                ]
    ...
    def train_categorical_ensemble(self,
        ...
        X_df = utils_categorical_ensembling.clean_categorical_definitions(
            X_df, categorical_column)
        ...
        if self.feature_learning == True:
            ...
            # If the user passed in a valid value for model_names (not None, and not a list where the only thing is None)
            if self.model_names is not None and not (len(
                    self.model_names) == 1 and self.model_names[0] is None):
                estimator_names = self.model_names
            else:
                estimator_names = self._get_estimator_names()

            X_df_transformed = self.fit_transformation_pipeline(
                X_df, y, estimator_names)
        ...
        for category in unique_categories:
            ...
            indices = list(
                np.flatnonzero(X_df[self.categorical_column] == category))
        ...
        for pair in sorted(
                categories_and_indices, key=lambda x: len(x[1]), reverse=True):
            category = pair[0]
            indices = pair[1]
            ...
            if len(indices) > self.min_category_size:
                categories_and_data.append(
                    [category, relevant_transformed_rows, relevant_y])
            ...
            # Otherwise, add it to our "all_small_categories" category, and train a model on all our small categories combined
                ...
                if isinstance(all_small_categories['relevant_transformed_rows'],
                              list):
                    all_small_categories[
                        'relevant_transformed_rows'] = relevant_transformed_rows
                else:
                    all_small_categories[
                        'relevant_transformed_rows'] = scipy.sparse.vstack(
                            [
        ...
        def train_one_categorical_model(category, relevant_X, relevant_y):
            ...
            try:
                ...
                if 'BinomialDeviance requires 2 classes' in str(
                        e
                ) or 'BinomialDeviance requires 2 classes' in e or 'BinomialDeviance requires 2 classes':
                    print('Found a category with only one label')
                    print('category: ' + str(category) + ', label: ' +
                          str(relevant_y[0]))
                    print(
                        'We will put in place a weak estimator trained on only this category/single-label, but consider some feature engineering work to combine this with a different category, or remove it altogether and use the default category when getting predictions for this category.'
                    )
                    ...
                    category_trained_final_model = self.train_ml_estimator(
                        ['RidgeClassifier'], self._scorer, relevant_X,
                        relevant_y)
                else:
                    raise

            self.trained_category_models[
                category] = category_trained_final_model
        ...
        if os.environ.get('is_test_suite', False) == 'True':
            ...
            results = list(
                map(lambda x: train_one_categorical_model(x[0], x[1], x[2]),
                    categories_and_data))
        else:
            ...
            try:
                results = list(
                    pool.map(
                        lambda x: train_one_categorical_model(x[0], x[1], x[2]),
                        categories_and_data))
            except RuntimeError:
        ...
        for result in results:
            if result['trained_category_model'] is not None:
                ...
                self.trained_category_models[category] = result[
                    'trained_category_model']
    ...
    def _join_and_print_analytics_results(self, df_feature_responses,
        ...
        df_results = df_results.head(n=100)
        print(tabulate(
            df_results, headers='keys', floatfmt='.4f', tablefmt='psql'))
        print('\n')
        print('*******')
        print('Legend:')
    ...
    def _print_ml_analytics_results_random_forest(self,
                                                  trained_model_for_analytics):
        try:
            final_model_obj = trained_model_for_analytics.named_steps[
                'final_model']
        except:
    ...
    def _get_trained_feature_names(self):
        ...
        trained_feature_names = self.transformation_pipeline.named_steps[
    ...
    def _print_ml_analytics_results_linear_model(self,
        ...
        print('\n\nHere are the results from our ' +
              final_model_obj.model_name + ' model')
        ...
        sorted_feature_summary = sorted(
            feature_summary, key=lambda x: abs(x[1]))
    ...
    def print_training_summary(self, gs):
        print(
            'The best CV score from our hyperparameter search (by default averaging across k-fold CV) for '
        ...
        if self.verbose:
            ...
            try:
                ...
                df_raw_scores = df_raw_scores.sort_values(
                ...
                cleaned_params = list(df_raw_scores['params'].apply(
                    utils.clean_params))
                df_params = pd.DataFrame(cleaned_params)
                df_scores = pd.concat(
                    [df_raw_scores.mean_score, df_params], axis=1)
                df_scores = df_scores.sort_values(
                    by='mean_score', ascending=True)
                print(
    ...
    def predict_uncertainty(self, prediction_data):
        ...
        predicted_vals = self.trained_pipeline.predict_uncertainty(
            prediction_data)
    ...
    def predict_intervals(self, prediction_data, return_type=None):
        ...
        return self.trained_pipeline.predict_intervals(
            prediction_data, return_type=return_type)
            ...
            if self.type_of_estimator == 'regressor':
                ...
                if self._scorer == accuracy_score:
                    ...
                    score, probas = self._scorer.score(
                        ...
                        y_test,
                        advanced_scoring=advanced_scoring)
                    utils_scoring.advanced_scoring_classifiers(
                        probas, y_test, name=self.name)
                    return score
                else:
                    return self._scorer.score(
                        self.trained_pipeline,
    ...
    def define_uncertain_predictions(self, base_predictions, y):
        if not (isinstance(base_predictions[0], float) or
                isinstance(base_predictions[0], int)):
    ...
    def score_uncertainty(self, X, y, advanced_scoring=True, verbose=2):
        ...
        score = utils_scoring.advanced_scoring_classifiers(
            df_uncertainty_predictions.uncertainty_prediction,
            is_uncertain_predictions)
    ...
    def save(self, file_name='auto_ml_saved_pipeline.dill', verbose=True):
        ...
        try:
            ...
            feature_names = self.trained_pipeline.named_steps[
                'dv'].get_feature_names()
        except AttributeError:
        ...
        if make_feature_importances == True:
            importances_dict = {}
            try:
                final_model = self.trained_pipeline.named_steps[
                    'final_model'].model
        ...
        model_name_map = {}
        if isinstance(self.trained_pipeline,
                      utils_categorical_ensembling.CategoricalEnsembler):
            for step in self.trained_pipeline.transformation_pipeline.named_steps:
                pipeline_step = self.trained_pipeline.transformation_pipeline.named_steps[
                    step]

                used_deep_learning = save_one_step(pipeline_step,
                                                   used_deep_learning)
        ...
        if verbose:
            ...
            if used_deep_learning == True:
                ...
                print(
                    'Specifically, we saved ' + str(len(model_name_map.keys()))
    ...
    def _train_ensemble(self, X_train, y_train):
        ...
        for idx, model_params in enumerate(self.ensemble_config):
            ...
            trained_model = self.train_ml_estimator(
        ...
        if ensemble_method != 'average' and self.type_of_estimator == 'classifier':
            print(
                'Because we are taking the minimum prediction for each class, these predicted probabilities are not expected to sum to 1 for each row'
            )
            print(
                'If you want the predicted probabilities to sum to 1, you should use ensemble_method="average"'
            )
            warnings.warn(
                'Predicted probabilities are not expected to add up to 1 if ensemble_method is not "average"'
            )
